...
## Tidying 
Next we'll need to do some tidying and add a few new variables. These include:
...
cfb_regular_play_2019 <- cfb_regular_play_2019 %>%
  ...
  mutate(pass = if_else(play_type == "Pass Reception" | play_type == "Passing Touchdown" |
         ...
         epa_success = ifelse((rush == 1 | pass == 1) & EPA >= 0.001, 1, 0),
...
## Viewing the top teams and players by average EPA
...
season_epa %>%
    ...
    head(5)
```
The first five offenses fit well with what we would expect based on other sources, such as the SP+ rankings. And while Clemson, Ohio State and Georgia also rank highly in other metrics, San Diego State and UAB rank 17th and 28th respectively in the opponent-adjusted SP+ ratings. That could suggest that opponent-adjusted EPA data would produce different results. 
